...
public class PredictThanosSnap {
    ...
    public static void main (String[] args) {
        ...
        StdRandom.setSeed(StdIn.readLong()); // set seed
        int vertices = StdIn.readInt();
        int[][] matrix = new int[vertices][vertices];

        // populate ajacency matrix
        ...
        // initialize variable for delete
        int delete = 0;
        int temp1 = 0;
        int temp2 = 0;

        while(temp1 < vertices){

            // delete if chance is less than 50%
            if(StdRandom.uniform() < 0.5){
                delete++;
                temp2 = 0;

                while(temp2 < vertices){
                    matrix[temp1][temp2] = 0;
                    matrix[temp2][temp1] = 0;
                    temp2++;
                }
            }
            temp1++;
        ...
        int remaining = vertices - delete;
        int count = 0;

        for(int i = 0; i < vertices; i++){
            for(int j = 0; j < vertices; j++){
                // counts number of connected edges left
                if(matrix[i][j] == 1){
                    count++;
                    break;
                }
