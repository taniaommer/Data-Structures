...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        while(count < numStudents){
            ...
            if(count == 0){
                ...
                SNode studentNode = new SNode(student, studentsInLine);
                studentsInLine = studentNode;
            }
    ...
    public void insertMusicalChairs () {
        ...
        SNode temp = new SNode(null, null);
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        for(int i = 0; i < n; i++){
            ...
            if(current == null){ // reached the end of the list, start from the beginning
                current = musicalChairs.getNext();
                previous = null;
            }
        }

        if(current.getStudent() == musicalChairs.getNext().getStudent()){ // student is at the front
            SNode first = musicalChairs.getNext();
            SNode second = musicalChairs;
            second.setNext(first.getNext());
            insertByHeight(current.getStudent());
        }
        else if(current.getStudent() == musicalChairs.getStudent()){ // student is at the end
            previous.setNext(current.getNext());
            musicalChairs = previous;
            insertByHeight(current.getStudent());
        }
        else{ // student is somewhere in the middle
            previous.setNext(current.getNext());
            insertByHeight(current.getStudent());
        }
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        if (studentsInLine == null) {   // no students in line yet
        ...
        if (previous == null) {     // studentToInsert is the shortest -> insert at the front
            studentsInLine = new SNode(studentToInsert, current);
            return;
        }

        if (current == null) {      // studentToInsert is the tallest -> insert at the end
            previous.setNext(new SNode(studentToInsert, null));
            return;
        }

        previous.setNext(new SNode(studentToInsert, current));  // studentToInsert is not the shortest or tallest -> insert in the middle
    ...
    public void eliminateLosingStudents() {
        ...
        int size = 0; 
        if (musicalChairs != null) {
            current = musicalChairs.getNext(); // start from the head node
            while (current != musicalChairs){
                size++;
                current = current.getNext();
            }
            current = musicalChairs;
        }
        while (size > 1){
        ...
        SNode firstStudent = musicalChairs.getNext();
        SNode currentStudent = firstStudent;
        while (currentStudent != musicalChairs) {
            currentStudent = currentStudent.getNext();
        }

        currentStudent.setNext(firstStudent);
    ...
    public void seatMusicalChairsWinner () {
        ...
        if (musicalChairs != null && musicalChairs.getNext() == musicalChairs) { // check if musicalChairs contains only one node
            Student winner = musicalChairs.getStudent();
            musicalChairs = null;
            int availableSeats = studentsSitting.length * studentsSitting[0].length;

            for (int i = 0; i < studentsSitting.length; i++) {
                for(int j = 0; j < studentsSitting[0].length; j++){
                    if (studentsSitting[i][j] == null && availableSeats > 0) {
